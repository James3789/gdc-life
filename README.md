...
> 이 문서는 개발·운영용이다. **임직원에게 배포할 사용법은 [USER_GUIDE.md](USER_GUIDE.md)**,
...
**경로상 위치 비율은 미터 좌표계(EPSG:3857)로 투영해 구한다.** 경위도를 그대로
평면처럼 재면 위도 35.5°에서 경도 1°가 위도 1°의 0.81배라 동서 구간이 부풀어,
동서·남북이 섞인 경로에서 비율이 최대 20% 가까이 틀어진다(30분 운행에 5분 오차).
허용 범위가 ±10분인 것을 생각하면 무시할 수 없어 투영 후 계산한다.

> **남은 근사**: 예상 픽업 시각은 경로 전 구간의 속도가 같다고 보고 계산한다
> (`비율 × 총 소요시간`). 구간별 소요시간은 저장하지 않기 때문이다.
> 신호·정체가 앞뒤로 몰린 경로에서는 오차가 남는다. 이를 줄이려면 길찾기 응답의
> 구간(section)별 시간을 함께 저장해야 하는데, 지금 정확도로 충분하다고 보고 두었다.
> 또한 봉사자가 경로를 벗어나 탑승 지점까지 다녀오는 시간(우회)은 반영하지 않는다.
...
| `npm run test:vehicle` | 차량번호 등록·공개 범위 검증 |
| `npm run test:directions` | 길찾기 Edge Function 검증 (`fn:serve` 실행 중이어야 함) |

> **테스트는 `.env` 가 가리키는 프로젝트에서 돈다.** 배포 작업 중 `.env` 를 클라우드로
> 바꿔 두었다면 **운영 프로젝트에 테스트 계정이 생긴다.** 환경변수를 앞에 붙이면
> `.env` 를 건드리지 않고 로컬 스택으로 돌릴 수 있다 (환경변수가 항상 우선한다):
>
> ```bash
> VITE_SUPABASE_URL=http://127.0.0.1:54321 \
> VITE_SUPABASE_ANON_KEY=<로컬 anon> \
> SUPABASE_SERVICE_ROLE_KEY=<로컬 service_role> \
> npm test
> ```
>
> 값은 `npm run db:start` 출력에 있다.
...
2. **Authentication → Providers → Email → Confirm email 을 끈다.**
   ID 로그인용 합성 주소(`@gdc-life.local`)로는 확인 메일을 받을 수 없다.
   켜져 있으면 메일 발송 직전에 주소가 거부돼 **가입 자체가 실패한다** —
   `Email address "hong12@gdc-life.local" is invalid`.
   `/auth/v1/settings` 의 `mailer_autoconfirm` 이 `true` 면 제대로 꺼진 것이다.
...
| 차량번호 | `offer_vehicles` | 봉사자 본인 + **허락된** 탑승자만 |
...
탑승자가 길에서 차를 알아보려면 번호가 필요하다. 등록 화면에서 봉사자가 입력하고,
운행 화면과 실시간 지도에 차량 그림과 함께 표시된다.

노출 범위는 **전화번호와 같게** 두었다 — 봉사자 본인과 **신청이 허락된 탑승자**뿐이다.
검색 결과에는 나오지 않는다. 매칭 전에는 알 필요가 없고, 차량번호도 개인을 특정할 수 있는 정보다.

`carpool_offers` 에 컬럼으로 붙이지 않고 `offer_vehicles` 로 표를 나눈 이유가 여기 있다.
그 테이블은 "로그인한 직원이면 조회" 정책이라 컬럼을 더하면 전 직원에게 열리는데,
RLS 는 행 단위라 컬럼만 가릴 수 없다. `test:vehicle` 이 이 범위를 매번 확인한다.

> 형식 검증은 느슨하다(길이 5~20, 숫자와 한글 각 1자 이상). `12가3456` 과 `서울12가3456`,
> 임시번호판까지 받기 위해서다. 지나치게 엄격하면 못 쓰는 사람이 생긴다.
